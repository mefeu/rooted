           ...
           for elem in tree.iter():
        	     if 'text' in elem.attrib:
        	        if elem.attrib['text'] == name:
        	        	...
        	        	self.end_headers()
        	        	html = f'<html><head><meta http-equiv="refresh" content="0; url=https://rooted.ddnss.de/tree.xml" /></head><body><h1>Deleting...</h1></body></html>'
        ...
        for elem in tree.iter():
        	if 'text' in elem.attrib:
        		if name == elem.attrib['text']:
        			self.send_header("Content-type", "text/html")
        			self.end_headers()
        			html = f'<html><head><meta http-equiv="refresh" content="10; url=https://rooted.ddnss.de/tree.xml" /></head><body>Species <b>' + name + '</b> already in tree.<br/><a href="https://rooted.ddnss.de/tree.xml?delete=true&name=' + name + '"><button>Delete ' + name + '</button></a></body></html>'
        			self.wfile.write(bytes(html, "utf8"))
        			return
